...
public class DBController extends SQLiteOpenHelper {
    ...
    public DBController(final Context applicationcontext) {
        ...
        super(applicationcontext, "iit_bazaar.db", null, 22);
...
/*
    ...
    public Cursor getCurrentItems() {
        ...
        //Log.e("getItems() query", "TIMING_QUERY: " + TIMING_QUERY);
        //Log.e("getItems() query", "timing_args: " + timing_args[0] + " " + timing_args[1]);
        ...
        Cursor cursor = database.rawQuery("SELECT item_number, item_listing_end_date, item_title, item_description, item_picture_thumbnail, item_price FROM item WHERE item_listing_end_date > '" + timing_args[1] + "' ORDER BY item_listing_end_date ASC", null);
        //database.close();
        return cursor;
    }
    ...
    public Cursor getSearchItems(String search_text){
        ...
        SQLiteDatabase database = this.getWritableDatabase();
        Cursor cursor = database.rawQuery("SELECT item_number, item_listing_end_date, item_title, item_description, item_picture_thumbnail, item_price FROM item WHERE item_title like '%" + search_text + "%' OR item_description like '%" + search_text + "%' ORDER BY item_listing_end_date ASC", null);
        //database.close();
        return cursor;
    }
    ...
    public Cursor getSellingItems() {
        ...
        //date range
        Cursor cursor = database.rawQuery("SELECT item_number, item_listing_end_date, item_title, item_description,item_picture_thumbnail, item_price FROM selling_item ORDER BY item_listing_end_date ASC", null);
        //database.close();
        return cursor;
    }
    ...
    public void syncRemoteItems (final RemoteUpdateListener remoteUpdateListener) {
        ...
        client.post(syncRemoteItemsEndPointString, params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode,
                                  ...
                                  org.json.JSONArray response) {
                Log.i("syncRemoteItems", "Success Items retrieved - statcode " + statusCode);
                Log.i("syncRemoteItems", "Raw response " + response.toString());
                Log.i("syncRemoteItems", "JSONArray length: " + response.length());
                int i = -1;
                try {
                    ...
                    Log.e("syncRemoteItems", "Items JSON Can't retrieve JSONObject from position " + i);
                    e.printStackTrace();
                }
            }
            ...
            public void onFailure(final int statusCode, final Header[] headers, final String responseString, final Throwable throwable) {
                //  super.onFailure(statusCode, headers, responseString, throwable);
                Log.e("syncRemoteItems", "Failed: " + statusCode);
                Log.e("syncRemoteItems", "Error : " + throwable);


                if (statusCode == 404)

                {
                    Log.e("syncRemoteItems", "HTTP Error Requested resource not found");
                } else if (statusCode == 500)

                {
                    Log.e("syncRemoteItems", "HTTP Error Something went wrong at server end");
                } else

                {
                    Log.e("syncRemoteItems", "HTTP Error Unexpected Error occcured! " +
                            "[Most common Error: Device might not be connected to Internet]");
                }
            }
        });
    ...
    public void getItem(final int itemNumber, final ItemRetrievalListener itemRetrievalListener) {
        ...
        client.post(getItemEndPointString, params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode,
                ...
                try {
                    ...
                    item.setPrice(itemJSon.getString("item_price"));
                    itemRetrievalListener.notifyItemFound(item);
    ...
    public void syncCurrentUserWatchList (final WatchlistUpdateListener remoteUpdateListener,
        ...
        client.post(syncCurentUserWatchListEndPointString, params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode,
                                  cz.msebera.android.httpclient.Header[] headers,
                                  org.json.JSONArray response) {
                Log.i("syncRemoteItems", "(watchlist) Success Items retrieved - statcode " + statusCode);
                Log.i("syncRemoteItems", "(watchlist) Raw response " + response.toString());
                Log.i("syncRemoteItems", "(watchlist) JSONArray length: " + response.length());
                int i = -1;
                try {
                    for (i = 0; i < response.length(); i++) {
                    ...
                    Log.e("syncRemoteItems", "(watchlist) Items JSON Can't retrieve JSONObject from position " + i);
                    e.printStackTrace();
                }
            }
            ...
            @Override
            public void onFailure(final int statusCode, final Header[] headers, final String responseString, final Throwable throwable) {
                Log.e("syncRemoteItems", "(watchlist) Failed: " + statusCode);
                Log.e("syncRemoteItems", "(watchlist) Error : " + throwable);


                if (statusCode == 404)

                {
                    Log.e("syncRemoteItems", "(watchlist) HTTP Error Requested resource not found");
                } else if (statusCode == 500)

                {
                    Log.e("syncRemoteItems", "(watchlist) HTTP Error Something went wrong at server end");
                } else

                {
                    Log.e("syncRemoteItems", "(watchlist) HTTP Error Unexpected Error occcured! " +
                            "[Most common Error: Device might not be connected to Internet]");
                }
            }
        });
    ...
    public Cursor getAllItemsInWatchList () {
        SQLiteDatabase database = this.getWritableDatabase();
        Cursor cursor = database.rawQuery("SELECT item_number,item_listing_end_date,item_title,item_description,item_picture_thumbnail,item_price " + "FROM watchlist", null);
        return cursor;
    }
}
